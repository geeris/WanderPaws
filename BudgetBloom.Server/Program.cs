...
using BudgetBloom.Implementation;
using BudgetBloom.Implementation.Auth.Queries;
using BudgetBloom.Implementation.Services;
using BudgetBloom.Implementation.Validation;
using BudgetBloom.Server.Core;
...
//builder.Services.AddDbContext<BudgetBloomContext>();

var connectionString = builder.Configuration.GetConnectionString("sqlConnectionString");
builder.Services.AddDbContext<BudgetBloomContext>(options => options.UseSqlServer(connectionString));
...
{
    c.SwaggerDoc("v1", new OpenApiInfo { Title = "Budget Bloom API", Version = "v1" });
...
if (app.Environment.IsDevelopment())
{
    ...
    app.UseSwaggerUI(c =>
    {
        c.SwaggerEndpoint("/swagger/v1/swagger.json", "Budget Bloom API v1");
    });
}
